...
class AssignmentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Formatear la fecha correctamente para datetime-local si estamos editando
        if self.instance and self.instance.pk and self.instance.due_date:
            # Convertir a hora local y formatear para datetime-local (YYYY-MM-DDTHH:MM)
            from django.utils import timezone
            local_time = timezone.localtime(self.instance.due_date)
            self.initial['due_date'] = local_time.strftime('%Y-%m-%dT%H:%M')
        
    def clean_due_date(self):
        due_date = self.cleaned_data.get('due_date')
        
        # Solo validar que sea futura si estamos creando una nueva asignación
        # Al editar, permitir mantener la fecha original incluso si ya pasó
        if due_date:
            # Si estamos creando (no hay instancia) o si la fecha cambió
            if not self.instance.pk or (self.instance.pk and self.instance.due_date != due_date):
                # Solo entonces validar que sea futura
