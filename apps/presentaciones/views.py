...
def grade_presentations_view(request):
    ...
    # Mostrar TODAS las presentaciones del profesor, sin filtrar por estado
    # Esto permite ver presentaciones en cualquier estado (UPLOADED, PROCESSING, ANALYZED, GRADED, FAILED, REJECTED)
    presentations = Presentation.objects.filter(
        assignment__course__teacher=request.user
    ).select_related(
        'student', 
