...
class Presentation(models.Model):
    ...
    def get_thumbnail_url(self):
        ...
        """
        # Si está en Cloudinary, regenerar URL de thumbnail correctamente
        if self.is_stored_in_cloud and self.cloudinary_public_id:
            from apps.ai_processor.services import CloudinaryService
            # Regenerar thumbnail URL usando el método correcto
            return CloudinaryService.get_video_thumbnail_url(self.cloudinary_public_id)
        elif self.cloudinary_thumbnail_url:
    ...
    def upload_to_cloudinary(self):
        ...
        try:
            ...
            if result:
                ...
                # Generar URL de miniatura automática usando el servicio de Cloudinary
                self.cloudinary_thumbnail_url = CloudinaryService.get_video_thumbnail_url(
                    result['public_id']
                )
...
class Participant(models.Model):
    ...
    def grade_color_class(self):
        """
        Retorna la clase CSS según el porcentaje de la calificación
        """
        if not self.ai_grade or not self.presentation.assignment:
            return 'grade-low'
        
        max_score = float(self.presentation.assignment.max_score)
        if max_score == 0:
            return 'grade-low'
        
        percentage = (float(self.ai_grade) / max_score) * 100
    ...
    def speaking_duration(self):
        """Retorna el tiempo de participación (alias de participation_time)"""
        return self.participation_time if self.participation_time else 0.0
    ...
    def __str__(self):
        max_score = self.presentation.assignment.max_score if self.presentation.assignment else 20
        return f"{self.label} - {self.presentation} ({self.ai_grade}/{max_score})"
    
    def _validate_coherence_level(self):
        """Valida y corrige el nivel de coherencia basándose en coherence_score"""
        score = self.coherence_score
        
        # Umbrales FIJOS (no afectados por configuración de rigurosidad)
        if score >= 90:
            correct_level = "Excelente"
        elif score >= 80:
            correct_level = "Muy Buena"
        elif score >= 70:
            correct_level = "Buena"
        elif score >= 60:
            correct_level = "Regular"
        elif score >= 50:
            correct_level = "Baja"
        else:
            correct_level = "Insuficiente"
        
        # Si el nivel es incorrecto, corregirlo
        if self.coherence_level != correct_level:
            self.coherence_level = correct_level
    
    def save(self, *args, **kwargs):
        """Override save para validar coherence_level antes de guardar"""
        self._validate_coherence_level()
